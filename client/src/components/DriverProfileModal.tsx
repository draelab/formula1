...
import { ChevronRight, Trophy, Flag, Timer, Gauge, Calendar, MapPin, Hash, Users } from "lucide-react";
...
export default function DriverProfileModal({
  ...
  if (!driver || !driver.career) return null;

  const teamColor = TEAM_COLORS[driver.team] || "#888";
  const carSpec = CAR_SPECS_2026.find((c) => c.team === driver.team);
  const constructor = CONSTRUCTORS_2026.find((c) => c.name === driver.team);
  const carImage = TEAM_CAR_IMAGES[driver.team];
  const { career } = driver;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogTitle className="sr-only">{driver.name} — Driver Profile</DialogTitle>
        ...
        <div className="relative overflow-hidden" style={{ backgroundColor: teamColor }}>
          ...
          <div className="flex items-end gap-6 p-6 pb-0">
            ...
            <div className="flex-1 pb-6 text-white">
              <div className="text-white/60 text-xs f1-mono uppercase tracking-widest mb-1">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mt-3 text-xs text-gray-400 f1-mono">
              <span className="flex items-center gap-1">
          ...
          <div>
            <h3 className="f1-display text-sm font-black text-[#1A1A2E] uppercase tracking-wide mb-3 flex items-center gap-2">
              <span className="w-1 h-4 bg-[#E8002D] inline-block" />
              Career Statistics
              <span className="text-gray-400 font-normal text-xs f1-mono ml-1">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
              {[
                ...
                <div key={label} className="bg-[#1A1A2E] rounded-sm p-3 text-center">
                  ...
                  <div className="text-[10px] text-white/40 f1-mono">{label}</div>
                </div>
              ))}
            </div>
            {career.championshipYears && (
              <div className="mt-2 text-xs text-gray-500 f1-mono">
            ...
            {career.previousTeams.length > 0 && (
              <div className="mt-2 text-xs text-gray-500 f1-mono flex items-center gap-1">
                <Users size={11} />
                Previous teams: {career.previousTeams.join(" → ")}
              </div>
            )}
            {career.juniorFormula && (
              <div className="mt-1 text-xs text-gray-400 f1-mono">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            {carSpec && (
              ...
              >
                <div className="text-[10px] text-gray-400 f1-mono uppercase tracking-widest mb-2">Current Car</div>
                {carImage && (
                ...
                <div className="text-xs text-gray-500 f1-mono mt-1">{carSpec.powerUnit}</div>
                <div className="text-xs text-gray-400 mt-1">{carSpec.totalPower} · {carSpec.weight}</div>
                <div className="flex items-center gap-1 text-xs font-medium mt-3 group-hover:gap-2 transition-all" style={{ color: teamColor }}>
            ...
            {constructor && (
              <button
                ...
                className="bg-white border border-gray-100 rounded-sm p-4 text-left hover:border-gray-300 transition-colors group cursor-pointer"
              >
                <div className="text-[10px] text-gray-400 f1-mono uppercase tracking-widest mb-2">Team</div>
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-2 h-10 rounded-full" style={{ backgroundColor: teamColor }} />
                  <div>
                    <div className="f1-display font-black text-[#1A1A2E] text-sm uppercase">{constructor.name}</div>
                    <div className="text-xs text-gray-500 f1-mono">
                      P{constructor.position} · {constructor.points} pts
                    </div>
                  </div>
                </div>
                <div className="text-xs text-gray-500 f1-mono">
