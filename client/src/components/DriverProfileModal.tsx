...
export default function DriverProfileModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="sm:max-w-4xl max-h-[85vh] overflow-y-auto p-0 gap-0 bg-background border-0"
      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Bio */}
          <div>
            <p className="text-sm text-muted-foreground leading-relaxed">{driver.bio}</p>
            <div className="flex items-center gap-4 mt-3 text-sm text-muted-foreground f1-mono">
              <span className="flex items-center gap-1">
          ...
          <div>
            <h3 className="f1-display text-sm font-black text-foreground uppercase tracking-wide mb-3 flex items-center gap-2">
              <span className="w-1 h-4 bg-[#E8002D] inline-block" />
              Career Statistics
              <span className="text-muted-foreground font-normal text-sm f1-mono ml-1">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
              {[
                ...
                <div key={label} className="bg-card rounded-sm p-3 text-center">
                  <div className="font-black f1-stat-number text-lg text-card-foreground">{value}</div>
                  <div className="text-xs text-muted-foreground f1-mono">{label}</div>
                </div>
              ))}
            </div>
            {career.championshipYears && (
              <div className="mt-2 text-sm text-muted-foreground f1-mono">
            ...
            {career.previousTeams.length > 0 && (
              <div className="mt-2 text-sm text-muted-foreground f1-mono flex items-center gap-1">
                <Users size={11} />
                Previous teams: {career.previousTeams.join(" → ")}
              </div>
            )}
            {career.juniorFormula && (
              <div className="mt-1 text-sm text-muted-foreground f1-mono">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            {carSpec && (
              <button
                ...
                className="bg-card border border-border rounded-sm p-4 text-left hover:border-muted-foreground/30 transition-colors group cursor-pointer"
              >
                <div className="text-xs text-muted-foreground f1-mono uppercase tracking-widest mb-2">Current Car</div>
                {carImage && (
                ...
                <div className="f1-display font-black text-foreground text-sm uppercase">{carSpec.chassis}</div>
                <div className="text-sm text-muted-foreground f1-mono mt-1">{carSpec.powerUnit}</div>
                <div className="text-sm text-muted-foreground mt-1">{carSpec.totalPower} · {carSpec.weight}</div>
            ...
            {constructorData && (
              <button
                ...
                className="bg-card border border-border rounded-sm p-4 text-left hover:border-muted-foreground/30 transition-colors group cursor-pointer"
              >
                <div className="text-xs text-muted-foreground f1-mono uppercase tracking-widest mb-2">Team</div>
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-2 h-10 rounded-full" style={{ backgroundColor: teamColor }} />
                  <div>
                    <div className="f1-display font-black text-foreground text-sm uppercase">{constructorData.name}</div>
